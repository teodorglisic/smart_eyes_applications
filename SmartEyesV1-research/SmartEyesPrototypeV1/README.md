...
This folder contains the core Swift and SwiftUI source for "Smart Eyes" — an iOS app that pairs with Meta smart glasses, streams the wearer's point-of-view camera to the phone, and sends captured photos to Gemini (via Firebase AI Logic) for spoken-question visual inspection. This is the original bachelor-thesis **research and testing track** build: it carries the empirical Phase 1-3 evaluation framework and structured telemetry export used to validate the approach, ahead of it being restructured into a use-case-driven MVP (see [`../SmartEyesMVP`](../../SmartEyesMVP)).
...
| `GeminiManager.swift` | Wraps Firebase AI Logic (Gemini) — the Phase 1-3 research prompt framework and single-shot analysis calls, with token telemetry. See below for how this works. |
...
This build's analysis is organized around the thesis's three-phase empirical testing track (`InspectionMode` enum), each with its own system prompt tuned for that phase's test conditions:
...
4. **Audit Log tab**: review past trials, rate accuracy, export telemetry for analysis.
...
Using the Meta Wearables DAT SDK means this app is subject to Meta's [Wearables Developer Terms](https://wearables.developer.meta.com/terms) and [Acceptable Use Policy](https://wearables.developer.meta.com/acceptable-use-policy); Meta may collect some data about how the app communicates with users' devices per their [Privacy Policy](https://www.meta.com/legal/privacy-policy/). Unlike the MVP build, this project has **not** opted out of that analytics collection — there's no `MWDAT.Analytics.OptOut` key in `Info.plist`, so it's on by default. See the [SDK repo's README](https://github.com/facebook/meta-wearables-dat-ios#opting-out-of-data-collection) if you want to disable it.
