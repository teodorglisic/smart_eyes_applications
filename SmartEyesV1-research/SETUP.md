...
2. Firebase console → **Project settings → Add app (iOS)**. Enter the app's bundle ID (see `SmartEyesPrototypeV1.xcodeproj` build settings).
3. Download the generated `GoogleService-Info.plist` and place it at `SmartEyesPrototypeV1/GoogleService-Info.plist` (next to `SmartEyesApp.swift`). This filename is already in `.gitignore`, so it will never be committed.
...
This is the earlier research/testing-track prototype (Phase 1-3 empirical evaluation framework, CSV/JSON telemetry export). See [`../SmartEyesMVP`](../SmartEyesMVP) for the current, use-case-driven client build.
